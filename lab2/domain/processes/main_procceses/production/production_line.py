...
class ProductionLine:
    ...
    def _assign_tasks(self) -> None:
        ...
        for machine in self._machines:
            if machine.is_available():
                finish_time: datetime = machine.start_production(
                    self._current_order.product
                )
    ...
    def notify_machine_finished(self, machine: ProductionUnit) -> None:
        ...
        if not self._current_order.quantity and all(
            machine.is_available() for machine in self._machines
        ):
