...
class Order:
    def __init__(self) -> None:
        ...
        self._status: Literal["pending", "paid", "shipped", "delivered", "cancelled"] = "pending"
    ...
    @status.setter
    def status(self, status: Literal["pending", "paid", "shipped", "delivered", "cancelled"]) -> None:
        self._status = status
    ...
    def remove_item(self, item: OrderItem) ->  None:
        if item not in self._items:
            raise OrderError(f"Позиции в заказе не существует")